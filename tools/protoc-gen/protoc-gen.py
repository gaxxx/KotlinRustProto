...
class RPC:
    ...
    byte[] result = null;
    executeCommand({number}, args.toByteArray(), new RustCore.ProtoCallback() {{
        @Override
        ...
        '''.format(input=self.get_input(), output=self.get_output(), name = self.method_name(), number = self.command_num)
...
{currentImport}
public abstract class {cls}Impl implements com.linkedin.android.rpc.{cls}Inf {{ 
...
'''.format(proto_file.name, __file__, cls=class_name, currentImport = current_import)]
        ...
        # Fill response
        f = response.file.add()
        f.name = base_dir + class_name + "Impl.java"
        f.content = "\n".join(file_contents)
        ...
        iface_contents = ["/*\n "
                         ...
                         "import androidx.annotation.Nullable;\n\n"
                         "import com.linkedin.android.rsdroid.RustCore;\n\n"
                         "import com.linkedin.android.proto.{backend};\n\n"
                         "public interface {}Inf {{".format(proto_file.name, __file__, class_name, backend=class_name)]
        for method in service_methods:
            iface_contents.append("\n" + str(method.as_interface()))
        iface_contents.append("\n}")
        iface = response.file.add()
        iface.name = base_dir + class_name + "Inf.java"
        iface.content = "\n".join(iface_contents)
